<form action="<?php echo admin_url('admin.php'); ?>" method="get" class="wps-inline" id="jquery-datepicker">
    ...
    if (isset($select_box)) {
        ?>
        <br/>
        <?php echo $select_box['title']; ?>:&nbsp;<select name="<?php echo $select_box['name']; ?>"
                                                          id="<?php echo $select_box['name']; ?>">
            <?php
            ...
            ?>
        </select><input type="submit" value="<?php _e('Select', 'wp-statistics'); ?>"
                        class="button-primary btn-danger wps-btn-inline"><br/>
        <?php
    }
    ?>
    ...
    <ul class="subsubsub wp-statistics-sub-fullwidth">
        ...
        foreach ($DateRang['list'] as $number_days => $value) {
            ?>
            <li class="all">
                <a <?php if ($value['active'] === true) { ?> class="current" <?php } ?>
        ...
        <input type="text" size="18" name="date-from" data-wps-date-picker="from"
               value="<?php echo $DateRang['from']; ?>" placeholder="YYYY-MM-DD" autocomplete="off">
        <?php _e('to', 'wp-statistics'); ?>
        <input type="text" size="18" name="date-to" data-wps-date-picker="to" value="<?php echo $DateRang['to']; ?>"
               placeholder="YYYY-MM-DD" autocomplete="off">
        <input type="submit" value="<?php _e('Go', 'wp-statistics'); ?>" class="button-primary">
        <input type="hidden" name="<?php echo \WP_STATISTICS\Admin_Template::$request_from_date; ?>" id="date-from"
               value="<?php echo $DateRang['from']; ?>">
        <input type="hidden" name="<?php echo \WP_STATISTICS\Admin_Template::$request_to_date; ?>" id="date-to"
               value="<?php echo $DateRang['to']; ?>">
</form>
<?php
