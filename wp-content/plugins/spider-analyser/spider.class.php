...
class WP_Spider_Analyser
{
    ...
    public static function parse_request()
    {
        ...
        $sql = "SELECT * FROM $t WHERE (status=4 OR status>10) AND (ip = '' OR ip LIKE %s) AND (name='' OR name = %s)";
    ...
    public static function vue_assets()
    {
        ...
        $assets = [];
        $vue_pack = WP_SPIDER_ANALYSER_PATH . '/tpl/index.html';
        do {
        ...
        switch ($op) {
            ...
            case 'top_spider':
                ...
                do {
                    ...
                    $sql = "SELECT COUNT(1) num,a.spider,b.status AS udg FROM $t a LEFT JOIN $t2 b ON a.spider=b.name WHERE DATE_FORMAT(a.visit_date,'%Y-%m-%d') $op '$ymd' GROUP BY a.spider ORDER BY num DESC LIMIT 10";
            ...
            case 'summary':
                ...
                do {
                    ...
                    self::cache($cache_param, $ret, 600);
            ...
            case 'path':
                ...
                do {
                    ...
                    if ($is_chart) {
                        ...
                        $sql = "SELECT url_type,COUNT(1) num FROM $t WHERE $where GROUP  BY url_type ";
                        $list = $wpdb->get_results($sql);
                    ...
                    $sql = "SELECT SQL_CALC_FOUND_ROWS COUNT(1) num,url,url_type,'' type,ROUND(COUNT(1)/$sum * 100,2) percent FROM $t WHERE $where GROUP BY url_md5 ORDER BY $order_by LIMIT $offset,$num";
            ...
            case 'ip':
                ...
                do {
                    ...
                    $sql = "SELECT SQL_CALC_FOUND_ROWS COUNT(1) num,a.spider,SUBSTRING_INDEX(a.visit_ip,'.',3) ip_range,ROUND(COUNT(1)/$sum * 100,2) percent,b.status AS udg FROM $t a LEFT JOIN $t2 b ON a.spider=b.name WHERE $where GROUP BY a.spider,ip_range ORDER BY $order_by LIMIT $offset,$num";
            ...
            case 'stop':
                ...
                do {
                    ...
                    if (isset($_POST['add']) && $_POST['add'] && is_array($_POST['add'])) {
                        ...
                        $cid = 4;
                        if (isset($_POST['cid']) && in_array($_POST['cid'], [11, 12, 13, 14, 15])) {
    ...
    public static function chart_data($day, $type, $compare = 0, $spider = null)
    {
        ...
        $sql = "SELECT COUNT(1) num,COUNT(DISTINCT spider) spider,DATE_FORMAT(visit_date,'$format') ymd $filed_more FROM $t WHERE DATE_FORMAT(visit_date,'%Y-%m-%d') $op '$ymd' $where_more GROUP BY ymd $group_more ORDER BY ymd";
        $list = $wpdb->get_results($sql);
        $tmp = [];
        foreach ($list as $r) {
            if ($type == 2) {
