...
<ul class="wfls-option wfls-option-toggled-select<?php if (!wfConfig::p() && isset($premium) && $premium) {
    ...
    <li id="<?php echo esc_attr($toggleID); ?>"
        class="wfls-option-checkbox<?php echo($toggleValue == $enabledToggleValue ? ' wfls-checked' : ''); ?>"
        role="checkbox" aria-checked="<?php echo($toggleValue == $enabledToggleValue ? 'true' : 'false'); ?>"
        tabindex="0"><i class="wfls-ion-ios-checkmark-empty" aria-hidden="true"></i></li>
    <li class="wfls-option-content">
        <ul id="<?php echo esc_attr($selectID); ?>">
            <li class="wfls-option-title"><span
            ...
            <li class="wfls-option-select wfls-padding-add-top-xs-small">
                <select<?php echo($toggleValue == $enabledToggleValue && !(!wfConfig::p() && isset($premium) && $premium) ? '' : ' disabled'); ?>
                    ...
                    <?php foreach ($selectOptions as $o): ?>
                        <option class="wfls-option-select-option"
