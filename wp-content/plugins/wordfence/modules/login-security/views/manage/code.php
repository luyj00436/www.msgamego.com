...
<div class="wfls-block wfls-always-active wfls-flex-item-full-width">
    ...
    <div class="wfls-block-content wfls-padding-add-bottom">
        <p>Scan the code below with your authenticator app to add this account. Some authenticator apps also allow you
            to type in the text version instead.</p>
        <div id="wfls-qr-code"></div>
        <p class="wfls-center wfls-no-bottom"><input id="wfls-qr-code-text" class="wfls-center" type="text"
                                                     value="<?php echo esc_attr($initializationData->get_base32_secret()); ?>"
                                                     onclick="this.select();" size="32" readonly></p>
    </div>
</div>
            ...
            function renderQrCode() {
                ...
                if (narrow !== narrowPreviously) {
                    $('#wfls-qr-code').empty().qrcode({
                        text: '<?php echo \WordfenceLS\Text\Model_JavaScript::esc_js($initializationData->get_otp_url()); ?>',
