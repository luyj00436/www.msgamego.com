insert
ignore into {wp_abj404_permalink_cache} (id, url, meta, post_parent, url_length)

/* This selects the permalink for a page ID. */
select subTable.*,
       length(subTable.url)

from (select wpp.id                                                                           as id,
                 ...
                 when wpp.post_type = 'post' then
                     concat(/* wpo_su.option_value, */
                             replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(
                                                                                             ...
                                                                                             '%hour%',
                                                                                             date_format(wpp.post_date, '%HH')),
                                                                                     '%minute%',
                                                                                     date_format(wpp.post_date, '%i')),
                                                                             '%second%',
                                                                             date_format(wpp.post_date, '%ss')),
