...
if (!function_exists('cao_widget_pay')) {
    function cao_widget_pay($args, $instance)
    {
        ...
        $cao_downurl2 = get_post_meta($post_id, 'cao_downurl2', true);
        ...
        if ($min_price < $cao_price || $cao_close_novip_pay) {
            ...
            if ($cao_close_novip_pay) {
                ...
                echo '<li><i class="fa fa-circle-o"></i> ' . $site_no_vip_name . '用户兑换所需 : <span class="pricing__opt">' . $cao_price . $site_money_ua . '</span></li>';
            }
            if ($min_price < $cao_price || $cao_close_novip_pay) {
                if ($CaoUser->vip_status()) {
                    echo '<li style="color: #21b3fc;"><i class="fa fa-check-circle"></i> ' . $site_vip_name . '会员兑换所需 : <span class="pricing__opt">' . ($cao_price * $cao_vip_rate) . $site_money_ua . '</span></li>';
                    $cao_this_am = ($cao_price * $cao_vip_rate) . $site_money_ua;
                } else {
                    echo '<li><i class="fa fa-circle-o"></i> ' . $site_vip_name . '会员兑换所需 : <span class="pricing__opt">' . ($cao_price * $cao_vip_rate) . $site_money_ua . '</span></li>';
                }
            }
            if ($cao_is_boosvip) {
                if (is_boosvip_status($user_id)) {
                    echo '<li style="color: #FF9800;"><i class="fa fa-check-circle"></i> 终身' . $site_vip_name . '兑换所需 : <span class="pricing__opt">免费</span></li>';
                    $cao_this_am = '免费获取';
                } else {
                    echo '<li><i class="fa fa-circle-o"></i> 终身' . $site_vip_name . '兑换所需 : <span class="pricing__opt">免费</span></li>';
                }
            }
            echo '</ul>';
        }
        ...
        echo '<span style=" padding: 5px;margin-top: 10px;margin-bottom:15px;display: block;background-color: #fffacd;color: #999999;border-radius: 4px;text-align: left;border: 1px dashed #ffc12b;font-size: 13px;"><p style="text-align: center;">————小贴士————</p>1、大部分游戏支持百度网盘/天翼云盘/迅雷等下载；<br/>2、请保存激活码，安装游戏需激活；<br/>3、非会员单独领取游戏有7天下载权，过期将无法下载；<br/>4、如遇下载安装问题，可在常见问题中查看教程或联系客服。</span>';
        $create_nonce = wp_create_nonce('caopay-' . $post_id);
        echo '<div class="pay-box">';

        $RiProPayAuth = new RiProPayAuth($user_id, $post_id);
        $cao_pwd_html = (empty($cao_pwd)) ? '' : '<span class="pwd"><span title="点击一键复制密码" id="refurl" class="copypaw" data-clipboard-text="' . $cao_pwd . '">' . $cao_pwd . '</span></span>';
        ...
        switch ($RiProPayAuth->ThePayAuthStatus()) {
            ...
            case 21: //登陆后  已经购买过 输出OK
                echo cao_get_post_downBtn($post_id); // 输出下载按钮
                $down2 = get_post_meta($post_id, 'cao_downurl2', true);
                if ($down2) {
                    echo '<a target="_blank" href="' . trim($down2) . '" class="btn btn--danger btn--block mt-10"><i class="fa fa-cloud-download"></i> 备用下载</a>';
                }
                echo $cao_pwd_html;
            ...
            <div class="author-fields">
                <div class="row">
                    <div class="col-6 text-center">
                        ...
                        <span class="d-block"><i class="fa fa-medium"></i> <?php echo $site_money_ua; ?>余额</span>
                    </div>
                    <div class="col-6 text-center">
                        <span class="num"><?php echo $CaoUser->get_consumed_balance(); ?></span>
                        <span class="d-block"><i class="fa fa-medium"></i> 已消费</span>
                    </div>
                </div>
            </div>
        </div>
...
if (!function_exists('magsy_about_widget')) {
    function magsy_about_widget($args, $instance)
    {
        ...
        ob_start(); ?>
        <img class="profile-image lazyload" data-src="<?php echo esc_url($profile_image); ?>"
            ...
            <?php foreach ($categories as $category) :
                ...
                <li class="category-item">
                    <a href="<?php echo esc_url(get_category_link($category->term_id)); ?>"
                       title="<?php echo esc_attr(sprintf(esc_html__('View all posts in %s', 'magsy'), $category->name)); ?>">
