...
add_action('wp_ajax_ajax_getcat_posts', 'ajax_getcat_posts');
add_action('wp_ajax_nopriv_ajax_getcat_posts', 'ajax_getcat_posts');
...
function user_login()
{
    ...
    if (is_wp_error($user_verify)) {
        ...
        exit;
        header("Location:https://www.msgamego.com/user?action=vip");
    }
    exit();
}
...
function check_pay()
{
    ...
    if ($status) {
        // 如果支付的是会员，则更新权限
        if ($post_id == 6 && !isset($_SESSION[$orderNum])) {
            $data = $ShopOrder->get_pay_info($orderNum);
            // 根据价格找到会员天数信息
            ...
            foreach ($vip_pay_setting as $key => $item) {
                if ((int)$item["price"] === (int)$order_price) {
                    $payInfo = $item;
                    break;
                }
            }
            $CaoUser = new CaoUser($uid);
            if (!$CaoUser->update_vip_pay($payInfo["daynum"])) {
                echo json_encode(array('status' => '0', 'msg' => '购买失败，请联系网站管理员'));
                exit;
            } else {
                $_SESSION[$orderNum] = 1;
                $msg = "购买成功，会员天数: " . $payInfo["daynum"];
