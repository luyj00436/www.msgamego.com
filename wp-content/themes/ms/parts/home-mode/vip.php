                    ...
                    <?php foreach ($home_mode_vip['vip_group'] as $item) : ?>
                        <div class="col-12 col-sm-6 col-md-4 col-lg-3">
                            <div class="vip-cell">
                                ...
                                <div class="price" style="color:<?php echo $item['_color']; ?>">
                                    <span><?php echo _cao('site_money_ua') ?></span><?php echo $item['_price']; ?></div>
                                <p><?php echo $item['_desc']; ?></p>
                                <?php if (is_user_logged_in()) : ?>
                                    <a href="<?php echo esc_url(home_url('/user?action=vip')); ?>" target="view_window"
                                       class="btn-sm primary" style="background:<?php echo $item['_color']; ?>"><i
                                                class="fa fa-user"></i> 前往开通</a>
                                <?php else: ?>
                                    <a class="login-btn btn-sm primary"
                                       style="background:<?php echo $item['_color']; ?>;cursor:pointer;"><i
                                                class="fa fa-user"></i> 登录购买</a>
                                <?php endif; ?>
                            </div>
                        </div>
                    <?php endforeach; ?>
